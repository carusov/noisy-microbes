...
result_path <- "~/projects/thesis/results/zymo_neat"
ref_path <- "~/projects/thesis/references"
analysis_path <- "~/projects/thesis/analysis/zymo_neat"
...
dada2_table <- as.tibble(dada2_table) %>% select(id, sample_names)
dada2_table
...
method_species <- sapply(all_table[, methods], function(m) sum(colSums(all_to_ref_species[m > 0,]) > 0))
...
summary_table <- data.frame(method = methods, total = method_counts, hits = method_hits, species = method_species,
