...
all_table <- matrix(0, nrow = 6, ncol = length(all_seqs))
row.names(all_table) <- methods
colnames(all_table) <- all_seqs

all_table["uclust", uclust_table$sequence] <- uclust_table$s160.MC.Neat
all_table["uparse", uparse_table$sequence] <- uparse_table$s160.MC.Neat
all_table["unoise", unoise_table$sequence] <- unoise_table$s160.MC.Neat
all_table["med", med_table$sequence] <- med_table$s160.MC.Neat
all_table["deblur", deblur_table$sequence] <- deblur_table$s160.MC.Neat
all_table["dada2", dada2_table$sequence] <- dada2_table$s160.MC.Neat

all_table <- collapseNoMismatch(all_table)
all_seqs <- colnames(all_table)
...
# all_to_ref <- outer(all_table$sequence, ref_seqs, levDist, band = -1)
# row.names(all_to_ref2) <- all_table$id

strains_to_ref <- collapse_group_dist(all_to_ref, strains)

# strains_to_ref <- t(aggregate(t(all_to_ref), list(strains), min))
...
all_table <- annotate_ref(all_table, all_to_ref, sample_names = methods, max_dist = 3)
...
summary_table <- summarize_seqs(all_table, all_to_ref, methods, strains)
colnames(summary_table)[1] <- "method"

# # summarize counts of various classes
# exp_strains <- rep(length(unique(strains)), length(methods))
# method_total <- colSums(all_table[, methods] > 0)
# method_strains <- sapply(all_table[, methods], function(m) sum(colSums(strains_to_ref[m > 0,]) > 0))
...
# all_table_gg <- all_table_gg %>% mutate(log10_count = log10(count))
# all_table_gg <- all_table_gg %>% group_by(method) %>% mutate(rel_count = count / sum(count)) %>% ungroup()
...
read_stats <- compute_pr_reads(all_table_gg, group = "method")
