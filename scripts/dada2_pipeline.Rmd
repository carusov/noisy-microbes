...
dada2_path <- file.path(data_path, "dada2")
setwd(data_path)

#map_file <- file.path(path, "map.txt")
...
save(derep_Fs, derep_Rs, dada_Fs.lrn, dada_Rs.lrn, dada_Fs, dada_Rs, file = file.path(dada2_path, "dadas.RData"))
...
There are a few sequences (3) with lengths outside the expected range for the V4 region, so I'm going to remove those.
**Revision:** Sequences outside the length range below have been filtered out by the `fastq_mergepairs` command in USEARCH, so the code below is not needed at this point. Do I want to make the merging less restrictive, potentially allowing a greater range of sequence lengths?
