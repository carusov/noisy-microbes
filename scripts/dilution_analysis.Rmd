...
uclust_table$reference <- uclust_table$dist_to_ref == 0
uparse_table$reference <- uparse_table$dist_to_ref == 0
unoise_table$reference <- unoise_table$dist_to_ref == 0
med_table$reference <- med_table$dist_to_ref == 0
deblur_table$reference <- deblur_table$dist_to_ref == 0
dada2_table$reference <- dada2_table$dist_to_ref == 0

uclust_table$ref_noisy <- isRefNoisy(uclust_table[, sample_names], uclust_to_ref, 1, 3)
uparse_table$ref_noisy <- isRefNoisy(uparse_table[, sample_names], uparse_to_ref, 1, 3)
unoise_table$ref_noisy <- isRefNoisy(unoise_table[, sample_names], unoise_to_ref, 1, 3)
med_table$ref_noisy <- isRefNoisy(med_table[, sample_names], med_to_ref, 1, 3)
deblur_table$ref_noisy <- isRefNoisy(deblur_table[, sample_names], deblur_to_ref, 1, 3)
dada2_table$ref_noisy <- isRefNoisy(dada2_table[, sample_names], dada2_to_ref, 1, 3)

uclust_table$ref_like <- uclust_table$reference | uclust_table$ref_noisy
uparse_table$ref_like <- uparse_table$reference | uparse_table$ref_noisy
unoise_table$ref_like <- unoise_table$reference | unoise_table$ref_noisy
med_table$ref_like <- med_table$reference | med_table$ref_noisy
deblur_table$ref_like <- deblur_table$reference | deblur_table$ref_noisy
dada2_table$ref_like <- dada2_table$reference | dada2_table$ref_noisy
...
isBlastHit <- function(seq_tab, blast_file){
...
isBlastNoisy <- function(seq_tab, blast_file, min_off = 1, max_off = 3){
  blast_tab <- as.tibble(read.table(blast_file, sep = "\t", comment.char = "#",
                                    col.names = c("seqID", "subjectIDs", "kingdom", "identity", "aln_length", "mismatches", 
                                                  "gapopens", "gaps", "qstart", "qend", "sstart", "send", "evalue", "bitscore")))
  ...
  blast_tab$cov_dist <- pmax(blast_tab$seq_len - blast_tab$aln_length, 0)  # compute number of terminal gaps, if any
  ...
  hits <- isBlastHit(seq_tab, blast_file)
  is_noisy[hits] <- FALSE
...
uclust_table$contaminant <- isBlastHit(uclust_table, file.path(analysis_path, "uclust_blast.txt")) & !uclust_table$reference
uclust_table$contam_noisy <- isBlastNoisy(uclust_table, file.path(analysis_path, "uclust_blast.txt"), 1, 3)
uclust_table <- uclust_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)

uparse_table$contaminant <- isBlastHit(uparse_table, file.path(analysis_path, "uparse_blast.txt")) & !uparse_table$reference
uparse_table$contam_noisy <- isBlastNoisy(uparse_table, file.path(analysis_path, "uparse_blast.txt"), 1, 3)
uparse_table <- uparse_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)

unoise_table$contaminant <- isBlastHit(unoise_table, file.path(analysis_path, "unoise_blast.txt")) & !unoise_table$reference
unoise_table$contam_noisy <- isBlastNoisy(unoise_table, file.path(analysis_path, "unoise_blast.txt"), 1, 3)
unoise_table <- unoise_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)

med_table$contaminant <- isBlastHit(med_table, file.path(analysis_path, "med_blast.txt")) & !med_table$reference
med_table$contam_noisy <- isBlastNoisy(med_table, file.path(analysis_path, "med_blast.txt"), 1, 3)
med_table <- med_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)

deblur_table$contaminant <- isBlastHit(deblur_table, file.path(analysis_path, "deblur_blast.txt")) & !deblur_table$reference
deblur_table$contam_noisy <- isBlastNoisy(deblur_table, file.path(analysis_path, "deblur_blast.txt"), 1, 3)
deblur_table <- deblur_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)

dada2_table$contaminant <- isBlastHit(dada2_table, file.path(analysis_path, "dada2_blast.txt")) & !dada2_table$reference
dada2_table$contam_noisy <- isBlastNoisy(dada2_table, file.path(analysis_path, "dada2_blast.txt"), 1, 3)
dada2_table <- dada2_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)
