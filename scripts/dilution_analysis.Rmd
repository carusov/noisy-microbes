...
snr_bees_facet <- ggplot(data = all_table_gg %>% filter(count > 0)) +
  geom_beeswarm(aes(x = sample, y = log10_count, color = factor(reference, levels = c(T, F))), cex = 0.15, dodge.width = 0.3) +
  labs(title = "Density distribution of log10(read counts) for reference and non-reference sequences",
...
class_comp <- ggplot(all_table_gg %>% filter(count > 0), aes(x = sample)) +
  ...
  scale_x_discrete(labels = dilution_labels) +
  labs(title = "Composition of dilution series communities by sequence class", x = "relative dilution") +
  theme(plot.title = element_text(size = 18, hjust = 0), axis.text.x = element_text(angle = 90))
class_comp
