...
dist_tables <- lapply(method_tables, function(mtab){
  seqs_to_ref <- outer(mtab$sequence, ref_seqs, levDist, band = -1)
  row.names(seqs_to_ref) <- mtab$id
  return(seqs_to_ref)
})

dist_strains_tables <- lapply(dist_tables, function(dtab){
...
# uclust_table$contaminant <- isBlastHit(uclust_table, uclust_blast) & !uclust_table$ref_like
# uclust_table$contam_noisy <- isBlastNoisy(uclust_table, uclust_blast, 1, 3) & !uclust_table$ref_like
# uclust_table <- uclust_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)
# 
# uparse_table$contaminant <- isBlastHit(uparse_table, uparse_blast) & !uparse_table$ref_like
# uparse_table$contam_noisy <- isBlastNoisy(uparse_table, uparse_blast, 1, 3) & !uparse_table$ref_like
# uparse_table <- uparse_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)
# 
# unoise_table$contaminant <- isBlastHit(unoise_table, unoise_blast) & !unoise_table$ref_like
# unoise_table$contam_noisy <- isBlastNoisy(unoise_table, unoise_blast, 1, 3) & !unoise_table$ref_like
# unoise_table <- unoise_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)
# 
# med_table$contaminant <- isBlastHit(med_table, med_blast) & !med_table$ref_like
# med_table$contam_noisy <- isBlastNoisy(med_table, med_blast, 1, 3) & !med_table$ref_like
# med_table <- med_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)
# 
# deblur_table$contaminant <- isBlastHit(deblur_table, deblur_blast) & !deblur_table$ref_like
# deblur_table$contam_noisy <- isBlastNoisy(deblur_table, deblur_blast, 1, 3) & !deblur_table$ref_like
# deblur_table <- deblur_table %>% mutate(contam_like = contaminant | contam_noisy, other = !ref_like & !contam_like)
# 
# dada2_table$contaminant <- isBlastHit(dada2_table, dada2_blast) & !dada2_table$ref_like
# dada2_table$contam_noisy <- isBlastNoisy(dada2_table, dada2_blast, 1, 3) & !dada2_table$ref_like
...
method_summary <- mapply(function(meth_tab, dist_tab, dist_sp_tab, snames){
  ...
  return(m_summary)
}, method_tables, dist_tables, dist_strains_tables, MoreArgs = list(snames = sample_names), SIMPLIFY = FALSE)
...
#   method_strains <- integer()
#   for (s in sample_names){
#     s_strains <- sum(apply(dist_strains_tables[[i]][method_tables[[i]][, s] > 0, ], 2, min) == 0)
#     method_strains <- c(method_strains, s_strains)
#   }
#   method_total <- colSums(method_tables[[i]][, sample_names] > 0)   # total number of sequences
#   method_refs <- colSums(method_tables[[i]]$reference & method_tables[[i]][, sample_names] > 0)
#   method_ref_noisy <- countNoisySeqs(method_tables[[i]][, sample_names], dist_tables[[i]], 1, 3)   # "ref_noisy" sequences
#   method_ref_like <- method_refs + method_ref_noisy[, "ref_noisy"]
#   method_contam <- colSums(method_tables[[i]]$contaminant & method_tables[[i]][, sample_names] > 0)  # perfect BLAST hits
#   method_contam_noisy <- colSums(method_tables[[i]]$contam_noisy & method_tables[[i]][, sample_names] > 0)  # one-off
#   method_contam_like <- method_contam + method_contam_noisy
#   method_other <- colSums(method_tables[[i]]$dist_to_ref > 3 & !method_tables[[i]]$contaminant & 
#                             !method_tables[[i]]$contam_noisy & method_tables[[i]][, sample_names] > 0)   # non-refs, not "ref_noisy"
#   method_pct_ref <- colSums(method_tables[[i]][method_tables[[i]]$reference, sample_names]) / colSums(method_tables[[i]][, sample_names])
#   method_pct_ref_noisy <- colSums(method_tables[[i]][method_tables[[i]]$ref_noisy, sample_names]) / colSums(method_tables[[i]][, sample_names])
#   method_pct_contam <- colSums(method_tables[[i]][method_tables[[i]]$contaminant, sample_names]) / colSums(method_tables[[i]][, sample_names])
#   method_pct_contam_noisy <- colSums(method_tables[[i]][method_tables[[i]]$contam_noisy, sample_names]) / colSums(method_tables[[i]][, sample_names])
#   method_pct_other <- colSums(method_tables[[i]][method_tables[[i]]$other, sample_names]) / colSums(method_tables[[i]][, sample_names])
#   m_summary <- data.frame(sample = sample_names, total = method_total, strains = method_strains,
#                           reference = method_refs, method_ref_noisy, ref_like = method_ref_like,
#                           contaminant = method_contam, contam_noisy = method_contam_noisy, contam_like = method_contam_like,
#                           other = method_other, 
...
seq_stats <- lapply(method_summary, function(msum, snames){
  ...
  # compute summary stats on reference and contaminant groups separately
  # TPR_ref <- msum$reference / (msum$reference + msum$ref_noisy)
  # PPV_ref <- msum$reference / (msum$reference + msum$other)
  # TPR_contam <- msum$contaminant / (msum$contaminant + msum$contam_noisy)
  
  mstat <- tibble(sample = snames, TP = TP, FN = FN, FP = FP,
                  recall = TPR, precision = PPV) 
                  # recall_ref = TPR_ref, precision_ref = PPV_ref, 
                  # false_neg_rate = FNR, false_disc_rate = FDR)
  return(mstat)
}, snames = sample_names)
...
dilution_summary_t <- mapply(function(stab, s_to_ref, s_to_ref_strains, meths){
  ...
  s_refs <- colSums(stab$reference & stab[, meths] > 0)
  ...
  s_pct_other <- colSums(stab[stab$other, meths]) / colSums(stab[, meths])

  # compile summaries into a table
  s_summary <- data.frame(method = meths, total = s_total, strains = s_strains,
                          reference = s_refs, s_ref_noisy, other = s_other, 
...
methods_gg <- mapply(function(mt, med, snames){
  mt_gg <- gather(mt, "sample", "count", one_of(snames))
  mt_gg <- mt_gg %>% mutate(log10 = log10(count))
  mt_gg <- mt_gg %>% group_by(sample) %>%
    mutate(norm = count / sum(count),
           norm_median = round(norm * med),
           log10_norm = log10(norm_median)) %>%
    ungroup()
  mt_gg <- mt_gg %>% select(id, sample, count, everything()) %>% select(-sequence, sequence)
  return(mt_gg)
}, mt = method_tables, med = medians, MoreArgs = list(snames = sample_names), SIMPLIFY = FALSE)
...
read_hist <- ggplot(all_table_gg %>% filter(sample == sname, count > min_count), 
  ...
  geom_vline(all_table_gg %>% filter(sample == sname, reference) %>% group_by(method) %>% summarise(min = min(count)), aes(xintercept = min)) +
  facet_wrap(~method) +
...
snr_box <- ggplot(data = all_table_gg %>% filter(method == mname)) +
  geom_boxplot(aes(x = sample, y = log10_norm, fill = factor(reference, levels = c(TRUE, FALSE)))) +
...
snr_violin <- ggplot(data = all_table_gg %>% filter(method == mname)) +
  geom_violin(aes(x = sample, y = log10_norm, fill = factor(reference, levels = c(T, F)))) +
...
snr_bees <- ggplot(data = all_table_gg %>% filter(method == mname)) +
  geom_beeswarm(aes(x = sample, y = log10_norm, color = factor(reference, levels = c(T, F))), dodge.width = 0.75) +
  labs(title = "Density distribution of log10(read counts) for reference and non-reference sequences",
...
seqs_line_non <- ggplot(sample_counts_gg, aes(x = sample, y = seqs)) +
...
seqs_line_norm2 <- ggplot(sample_counts_gg[sample_counts_gg$sample %in% sample_names[1:7], ], aes(x = sample, y = seqs_norm_method_median)) +
...
snr_box_contam <- ggplot(all_table_gg %>% filter(method == mname, !ref_like), aes(x = sample, y = log10_norm)) +
...
summary_bars <- ggplot(data = all_summary_gg %>% filter(statistic %in% c("pct_ref", "pct_ref_noisy", "pct_contam", "pct_contam_noisy", "pct_other")), 
                       aes(x = sample)) +
