...
deblur_seqs <- toupper(deblur_table$sequence)
...
deblur_table <- deblur_table %>% select(id, sample_name, sequence)
...
dada2_table <- dada2_table %>% select(id, sample_name, sequence)
...
#species <- lapply(str_split(names(ref_seqs), "_"), `[`, 1:2)
#species <- sapply(species, paste, collapse = "_")
...
method_strains <- sapply(all_table[, methods], function(m) sum(colSums(all_to_ref_strains[m > 0,]) > 0))
...
write_tsv(read_stats, path = file.path(analysis_path, "schirmer_read_stats.tsv"))
...
snr_box <- ggplot(data = all_table_gg, aes(x = method, y = log10)) +
...
snr_violin <- ggplot(data = all_table_gg, aes(x = method, y = log10)) +
