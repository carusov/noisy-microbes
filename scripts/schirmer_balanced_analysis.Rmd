...
snr_box <- ggplot(data = all_gg, aes(x = method, y = log10, fill = factor(hit, levels = c(TRUE, FALSE)))) +
  geom_boxplot(aes(x = method, y = log10, fill = factor(hit, levels = c(TRUE, FALSE)))) +
#  geom_jitter(width = 0.25, height = 0.25, aes(color = factor(hit, levels = c(TRUE, FALSE))), alpha = 0.5) +
...
#  scale_color_manual(name = "sequence identity", labels = c("reference", "non-reference"), values = c("green", "red")) +
  theme(legend.position = c(0.95, 0.05),
        legend.justification = c(1, 0))
snr_box

# ref vs. non-ref violin plots, raw counts
snr_violin <- ggplot(data = all_gg, aes(x = method, y = log10)) +
#  geom_violin(aes(x = method, y = log10, fill = factor(hit, levels = c(T, F)))) +
  geom_beeswarm(aes(color = factor(hit, levels = c(T, F))), dodge.width = 0.75) +
