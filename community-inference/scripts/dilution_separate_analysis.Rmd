...
ref_seqs <- readDNAStringSet(file.path(zymo_ref, ref_fastas)) %>% as.character()
ref_strains <- sapply(str_split(names(ref_seqs), "_16S"), `[`, 1)
...
strain_dist_tables <- lapply(ref_dist_tables, collapse_group_dist, ref_strains)
...
method_summary <- mapply(summarize_seqs, method_tables, ref_dist_tables, 
                         MoreArgs = list(strains = ref_strains, sample_names = sample_names, max_dist = max_dist),
                         SIMPLIFY = FALSE)
...
ref_colors <- brewer.pal(12, "Paired")[c(2, 1)] #, brewer.pal(8, "Dark2")[2])
...
non_reflike_seq_lines <- dilution_line_plot(all_table_gg, "class", !ref_like, "count", size = 1) +
  scale_color_manual(name = "Sequence class   ", values = contam_colors, labels = paste(classes[3:5], "   ")) +
  ylab("sequences") +
  facet_wrap(~method) +
  big_labels(title = 20, text = 18, legend.position = "top") +
...
reflike_seq_lines <- dilution_line_plot(all_table_gg, "class", ref_like, "count", size = 1) +
  scale_color_manual(name = "Sequence class   ", values = ref_colors, labels = paste(classes[1:2], "   ")) +
  ylab("sequences") +
  facet_wrap(~method) +
  big_labels(title = 20, text = 18, legend.position = "top") +
  guides(color = guide_legend(override.aes = list(size = 3)))
...
non_reflike_tax_lines <- dilution_tax_line_plot(taxa_summary_gg, "class", 
                                                c("Contaminant", "Contam Noisy", "Other"), "identity") +
...
reflike_tax_lines <- dilution_tax_line_plot(taxa_summary_gg, "class", c("Reference", "Ref Noisy"), "identity") +
...
ggarrange(reflike_tax_lines, non_reflike_tax_lines, labels = c("A", "B"), nrow = 2)
...
snr_box_facet <- ggplot(data = all_table_gg %>% filter(count > 0)) +
  ...
  labs(x = "dilution", y = "log10(abundance)") +
...
class_seqs_comp <- ggplot(all_table_gg %>% filter(count > 0), aes(x = sample)) +
  geom_bar(aes(fill = fct_rev(class)),
           width = 0.8) +
  facet_wrap(~method) +
  ...
  labs(x = "dilution", y = "sequences") +
  big_labels(title = 20, text = 18, legend.position = "right") +
  theme(panel.spacing = unit(1.5, "lines")) +
