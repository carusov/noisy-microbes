...
plot_path <- file.path(analysis_path, "plots", "manuscript")
...
class_reads_comp <- ggplot(all_table_gg, aes(x = sample)) +
  ...
  big_labels(title = 10, text = 9, legend.position = "top") +
...
#   geom_col(aes(y = count, fill = fct_rev(class)4), position = position_fill(), width = 0.7) +
#   scale_x_discrete(labels = dilution_labels) +
...
(seqs_line_norm <- ggplot(sample_counts_gg, aes(x = sample, y = seqs_norm_method_median)) +
  ...
  #       legend.text = element_text(margin = margin(r = 7))) +
