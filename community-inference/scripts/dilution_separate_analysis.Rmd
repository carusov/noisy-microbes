...
ggarrange(class_seqs_comp, class_reads_comp, labels = c("A", "B"), vjust = c(1.5, 0.5), nrow = 2, 
          font.label = list(size = 24), heights = c(5, 4))
...
reflike_seq_lines <- dilution_line_plot(all_table_gg, "class", ref_like, "count", size = 1) +
...
non_reflike_seq_lines <- dilution_line_plot(all_table_gg, "class", !ref_like, "count", size = 1) +
...
ggarrange(reflike_seq_lines, non_reflike_seq_lines, labels = c("A", "B"), nrow = 2, font.label = list(size = 24))
...
(snr_box_facet <- ggplot(data = rbind(all_table_gg, deblur_one),
  ...
  geom_jitter(aes(fill = factor(reference, levels = c(TRUE, FALSE))),
              ...
              shape = 21, size = 1.6, stroke = 0.8, color = "black", alpha = 1, show.legend = FALSE) +
  ...
  big_labels(title = 36, text = 30, legend.position = "top") +
  theme(panel.spacing = unit(1.5, "lines"),
        legend.margin = margin(b = 45)) +
  guides(fill = guide_legend(keywidth = 3, keyheight = 4.5)) +
  # big_labels(title = 16, text = 12, legend.position = "top") +
  # theme(panel.spacing = unit(0.5, "lines"),
  #       legend.margin = margin(b = 10)) +
  # guides(fill = guide_legend(keywidth = 1, keyheight = 1, override.aes = list(alpha = 1))) +
...
alpha_dots <- ggplot(alpha_div_gg %>% filter(method != "Expected", sample != "expected"),
                     aes(x = sample)) +
  geom_point(aes(y = diversity, color = method), size = 18) +
  geom_hline(data = alpha_div_gg %>% filter(sample == "expected"), 
             aes(yintercept = diversity), color = "black", show.legend = TRUE) +
  geom_line(aes(y = diversity, color = method, group = method), size = 2) +
  scale_color_manual(name = "Method  ", values = method_colors, labels = paste(levels(alpha_div_gg$method), "   ")) +
  scale_x_discrete(labels = dilution_labels) +
  labs(y = "alpha diversity") +
  facet_wrap(~alpha_index, nrow = 1, scales = "free_y") +
  big_labels(title = 72, text = 60, legend.position = "top") +
  guides(color = guide_legend(keywidth = 6, keyheight = 6, nrow = 1)) + 
...
seqs_line_norm <- ggplot(sample_counts_gg, aes(x = sample, y = seqs_norm_method_median)) +
  geom_point(aes(color = method), size = 12) +
  geom_line(aes(group = method, color = method), size = 4) +
  ...
  scale_color_brewer(name = "Method   ", labels = paste(levels(sample_counts_gg$method), "   "), palette = "Dark2") +
  scale_x_discrete(labels = dilution_labels) +
  big_labels(title = 72, text = 60, legend.position = "top") +
  theme(panel.spacing = unit(3, "lines"),
        legend.margin = margin(b = 90),
        plot.margin = unit(c(2, 0, 4, 0), "lines")) +
  guides(color = guide_legend(nrow = 1, keywidth = 8))
...
Define a threshold filter to demonstrate the effectiveness of this strategy for removing contaminants.
```{r threshold filter}

options(tibble.print_max = 100, tibble.print_min = 100)

all_nested <- rbind(all_table_gg, deblur_zero) %>% group_by(method, sample) %>% nest()

# Define a dplyr-friendly function that computes confusion matrix cell values
# and performance metrics for each value in a vector of cutoffs
filter_stats <- function(dt, cutoffs){
  cutoffs <- enquo(cutoffs)
  cutoffs <- dt %>% pull(!!cutoffs)
  map_df(cutoffs, function(x) {
    predictor <- dt %>% pull(rel_count)
    predicted <- factor(predictor > x, levels = c(TRUE, FALSE))
    true <- factor(dt$reference, levels = c(TRUE, FALSE))
    cm <- confusionMatrix(predicted, true)$table
    t <- "TRUE"
    f <- "FALSE"
    summary <- tibble(threshold = x,
                      TP = cm[t, t],
                      FN = cm[f, t],
                      FP = cm[t, f],
                      TN = cm[f, f],
...
# sts <- filter_stats(all_nested$data[[1]], rel_count)
  
filter_sweep <- all_nested %>% 
  mutate(filter_stats = map(data, ~filter_stats(., rel_count))) %>%
  select(method, sample, filter_stats) %>%
  unnest() %>%
  mutate(sens_spec = sensitivity + specificity,
         F1 = 2 * (sensitivity * precision) / (sensitivity + precision))

filter_best <- filter_sweep %>% 
  group_by(method, sample) %>%
  mutate(max_sens_spec = sens_spec == max(sens_spec, na.rm = TRUE),
         max_F1 = F1 == max(F1, na.rm = TRUE)) %>%
  filter(max_sens_spec | max_F1) %>%
  ungroup()
filter_best <- rbind(filter_sweep %>% 
                       group_by(method, sample) %>%
                       filter(sens_spec == max(sens_spec, na.rm = TRUE)) %>%
                       mutate(optimized_by = "SENS_SPEC") %>%
                       ungroup(),
                     filter_sweep %>%
                       group_by(method, sample) %>%
                       filter(F1 == max(F1, na.rm = TRUE)) %>%
                       mutate(optimized_by = "F1") %>%
                       ungroup())

se <- function(x) sqrt(var(x) / length(x))

filter_best_summary <- filter_best %>% 
  group_by(optimized_by, sample) %>%
  summarise(mean_threshold = mean(threshold),
            se_threshold = se(threshold),
            mean_sensitivity = mean(sensitivity),
            se_sensitivity = se(sensitivity),
            mean_specificity = mean(specificity),
            se_specificity = se(specificity),
            mean_precision = mean(precision),
            se_precision = se(precision),
            mean_FPR = mean(FPR),
            se_FPR = se(FPR),
            mean_sens_spec = mean(sens_spec),
            se_sens_spec = se(sens_spec),
            mean_F1 = mean(F1),
            se_F1 = se(F1)) %>%
  ungroup() %>%
  mutate(mean_opt_stat = ifelse(optimized_by == "SENS_SPEC", mean_sens_spec, mean_F1),
         se_opt_stat = ifelse(optimized_by == "SENS_SPEC", se_sens_spec, se_F1))

pd = position_dodge(0.3)

(opt_thresholds <- ggplot(filter_best_summary, 
                          aes(x = sample, y = mean_threshold, group = optimized_by, color = optimized_by)) +
    geom_errorbar(aes(ymin = mean_threshold - se_threshold, ymax = mean_threshold + se_threshold), 
                size = 0.5, width = 0.15, position = pd) +
    geom_line(size = 0.5, position = pd) +
    geom_point(size = 3, position = pd) +
    scale_color_manual(name = "Optimized by", values = c("royalblue3", "tan3"),
                       labels = c("F1 score", "sensitivity + specificity")) +
    facet_wrap(~optimized_by, nrow=2) +
    scale_x_discrete(labels = dilution_labels) +
    # scale_y_log10() +
    labs(x = "sample", y = "optimized threshold mean +/- SE") +
    big_labels(text = 14, legend.position = "right"))

pj = position_jitter(height = 0.0, width = 0.2, seed = 4)

(method_thresholds <- ggplot(filter_best) +
    geom_point(aes(x = sample, y = threshold, color = method), position = pd) +
    geom_line(aes(x = sample, y = threshold, color = method, group = method), position = pd) +
    facet_wrap(~optimized_by, nrow = 2) +
    big_labels(text = 14, legend.position = "right"))

(opt_stats <- ggplot(filter_best_summary, aes(x = sample, y = mean_opt_stat, color = optimized_by, group = optimized_by)) +
    geom_errorbar(aes(ymin = mean_opt_stat - se_opt_stat, ymax = mean_opt_stat + se_opt_stat),
                  size = 0.5, width = 0.2, position = pd) +
    # geom_line(position = pd) +
    geom_point(size = 2, position = pd) +
    facet_wrap(~optimized_by, scales = "free_y", nrow = 2) +
    # scale_y_continuous(limits = c(0.7, 2.2), breaks = seq(0.6, 2.2, 0.2)) +
    scale_color_manual(name = "Optimized statistic", values = c("royalblue3", "tan3"), 
                       labels = c("F1 score", "sensitivity + specificity")) +
    labs(x = "sample", y = "optimized filter statistic mean +/- SE") +
    big_labels(text = 12, legend.position = "top"))

# (opt_sens_spec <- ggplot(filter_best_summary %>% filter(optimized_by == "SENS_SPEC"), 
#                          aes(x = method, y = mean_sens_spec, group = optimized_by)) +
#     geom_errorbar(aes(ymin = mean_sens_spec - se_sens_spec, ymax = mean_sens_spec + se_sens_spec),
#                   size = 0.5, width = 0.2, position = pd, color = "tan3") +
#     geom_line(position = pd, color = "tan3") +
#     geom_point(size = 2, position = pd, color = "tan3") +
#     scale_y_continuous(limits = c(1.8, 2.1), breaks = seq(1.8, 2.1, 0.1)) +
#     scale_color_manual(name = "Optimized statistic", values = "tan3", 
#                        labels = "sensitivity + specificity") +
#     labs(x = "method", y = "sensitivity + specificity") +
#     big_labels(legend.position = "top"))
# 
# (opt_F1 <- ggplot(filter_best_summary %>% filter(optimized_by == "F1"), 
#                          aes(x = method, y = mean_F1, group = optimized_by)) +
#     geom_errorbar(aes(ymin = mean_F1 - se_F1, ymax = mean_F1 + se_F1),
#                   size = 0.5, width = 0.2, position = pd, color = "royalblue3") +
#     geom_line(position = pd, color = "royalblue3") +
#     geom_point(size = 2, position = pd, color = "royalblue3") +
...
# opt_stats <- ggarrange(opt_sens_spec, opt_F1, nrow = 2)
ggarrange(opt_thresholds, opt_stats, labels = c("A", "B"), vjust = c(1.5, 1.5), 
          ncol = 2, font.label = list(size = 18))

ggplot(filter_best %>% filter(optimized_by == "F1"), aes(x = sample, y = F1, color = method, group = method)) +
