...
plot_path <- "~/thesis/analysis/high_biomass/plots/manuscript"
...
comp_bar <- ggplot(all_table_gg %>% filter(dataset == dname), aes(x = method, y = count)) +
  ...
  scale_fill_identity(name = "ASV/OTU", guide = "legend", 
                      labels = c(paste("Ref", 1:9), rep("non-Ref", length(other_colors))), 
                      breaks = c(ref_colors[1:9], other_colors)) +
  scale_x_discrete(labels = methods) +
  labs(title = paste("Relative composition of ", dname, " dataset processed with different methods"), 
       # subtitle = paste("Dilution series processed with", mname), 
       x = "ASV/OTU method", y = "fraction of sample") +
...
damore_dist_file <- "damore_proportions.tsv"
...
damore_props <- read_tsv(file.path(ref_path, damore_prop_file), col_names = c("name", "proportion"))
...
alpha_dots <- ggplot(alpha_div_gg %>% filter(alpha_index %in% c("Shannon", "Inverse Simpson", "Fisher")),
  ...
  geom_point(aes(y = diversity, fill = dataset, color = expected), size = 18, stroke = 4) +
  # geom_point(aes(y = diversity, color = dataset), size = 3) +
  # geom_hline(data = alpha_div_gg %>% filter(method == "Expected"), 
  #            aes(yintercept = diversity, color = dataset), size = 4, show.legend = TRUE) +
  labs(y = "alpha diversity") +
  # scale_color_manual(name = "Dataset  ", values = alpha_colors, labels = paste(levels(alpha_div_gg$dataset), "   ")) +
  scale_fill_manual(name = "Dataset  ", values = alpha_colors, labels = paste(levels(alpha_div_gg$dataset), "  "), guide = T) +
  scale_shape_manual(name = " Source  ", values = c(21, 24), labels = paste(c("Data", "Expected"), "  ")) +
  ...
  guides(fill = guide_legend(keywidth = 6, keyheight = 6, override.aes = list(shape = 21, color = "white")), 
         shape = guide_legend(keywidth = 6, keyheight = 6)) +
...
(snr_box_facet <- ggplot(data = rbind(all_table_gg, deblur_zero),
  ...
  geom_jitter(aes(fill = factor(reference, levels = c(TRUE, FALSE))),
              ...
              shape = 21, size = 1.6, stroke = 0.8, color = "black", alpha = 0.7, show.legend = FALSE) +
  scale_fill_manual(values = c("deepskyblue", "gold1")) +
  facet_wrap(~dataset) +
  labs(x = "method", y = expression(bold("log"[10]*" abundance"))) +
  # scale_color_manual(values = c("blue", "orange"), guide = FALSE) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
        labels = trans_format("log10", math_format(10^.x))
        # limits = c(10^0, max(all_table_gg$count))
        ) +
  ...
  # big_labels(title = 16, text = 14, legend.position = "top") +
